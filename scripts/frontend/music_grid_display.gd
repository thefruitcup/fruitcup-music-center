...
func update_grid() -> void:
	...
	var current_sub_grid_container :GridContainer= GridContainer.new()
	...
	for file : String in Global.all_music_files_loaded:
		...
		button.label.theme = (get_parent() as Control).theme
		
		button.button_down.connect(Global.on_audio_file_clicked.bind(file))
		
		if add_gradient: button.viewport_container.material = shader_material
